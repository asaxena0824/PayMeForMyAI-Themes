# PayMeForMyAI

## Introduction

PayMeForMyAI is a platform that enables creators to build and monetize AI apps. AI Creators can charge their users per chat session, allowing them to generate revenue from their chatbots.

To handle and receive payments, App creators can integrate their bank accounts or debit cards using Stripe. The revenue generated from interactions with their bot is returned to the creator entirely.

There are no limits on the number of chats, and users don't need to have ChatGPT Plus to use any chatbot app on PayMeForMyAI.

## How to monetize your AI bot or GPT?

* Sign up or Log in on [PayMeForMyAI](https://www.paymeformyai.com/signup).
* Connect your Stripe account to receive payouts.
* Click on `Create App` from the dashboard.
* Fill out the form to create your own GPT bot.
* Click on `Test` to test the bot or on `Launch` to launch the bot you created.
* Your bot is now launched! Click on it to use it, and share the link with others.

## Themes

### Traditional

<img width="983" alt="Screenshot 2024-01-28 at 9 27 19 PM" src="https://github.com/asaxena0824/PayMeForMyAI-Themes/assets/121717018/113f29f3-6845-4d8d-924b-f9b24eed1082">

<img width="879" alt="Screenshot 2024-01-28 at 9 27 40 PM" src="https://github.com/asaxena0824/PayMeForMyAI-Themes/assets/121717018/27e91d9b-0941-440a-a37a-a6f260e1f812">

### Dark

<img width="1435" alt="Screenshot 2024-01-28 at 9 28 09 PM" src="https://github.com/asaxena0824/PayMeForMyAI-Themes/assets/121717018/c56930d6-36b7-499b-aeae-2b6c686496ea">

![image](https://github.com/asaxena0824/PayMeForMyAI-Themes/assets/121717018/871c247f-3162-40b2-9d6f-f92950353be9)

### Dark Chat

<img width="1107" alt="Screenshot 2024-02-08 at 2 01 19 AM" src="https://github.com/asaxena0824/PayMeForMyAI-Themes/assets/121717018/189d9618-6fb3-403a-bb6b-40dc5f0a4713">
